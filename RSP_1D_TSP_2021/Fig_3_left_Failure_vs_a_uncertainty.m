...
nbSigmaA2 = 10;
sa2_set = logspace(-6, 0, nbSigmaA2).';


nbA    = 10;         % nb of MC for a(x)


ratio_rho = .01;    % ratio between l_n and l_a => ratio_rho = l_n / l_a, with l_N: length scale of the spatial noise


nbMcMc = 50;        % nb of MC simulations for each configuratio, i.e. pair (a(x), uncertainty on \hat{a}(x))
...
for inda = 1:nbA    % loop MC on a(x)
    ...
    for indUncertainty = 1:nbSigmaA2     % loop on uncertainty on a(x)
        ...
        for Mont = 1:nbMcMc     % loop MC on \hat{a}(x)
            ...
            plt = plot(x,a,'LineWidth',2.5);
            plt.Color = axs.ColorOrder(1,:);
            plt = plot(xSelected, aSelected, 'o','MarkerSize', 12,'LineWidth',2);
            plt.MarkerEdgeColor = axs.ColorOrder(1,:);
            plt.HandleVisibility = 'off';
            
            plt = plot(x, ma,'LineWidth',2.5);
            plt.Color = axs.ColorOrder(2,:);
            plt = plot(xSelected, maSelected, 'o','MarkerSize', 12,'LineWidth',2);
            plt.Color = axs.ColorOrder(2,:);
            plt.HandleVisibility = 'off';
            
            grid on
            set(gca,'FontSize',20);            
            lbl = xlabel('$x$', 'FontSize',28);
            lbl.Interpreter = 'latex';
            lbl = ylabel('$a(x)$', 'FontSize',28);
            lbl.Interpreter = 'latex';
            %                 axs.FontSize = 28;
            lgd = legend({'$a(x)$' ; '$\hat{a}(x)$'}, 'FontSize',26);
            ...
            figure(3); clf
            axs = axes;
            plot(x, 10*log10(SNR_optimal), 'LineWidth', 2.5); hold on
            plt = plot(xSelected, 10*log10(SNR_optimal(indSelected)), 'o','MarkerSize', 12, 'LineWidth', 2);
            plt.MarkerEdgeColor = axs.ColorOrder(1,:);
            plt.HandleVisibility = 'off';
            plt = plot([0 1], 10*log10(SNR_optimal(indSelected(1)))*ones(2,1), '--', 'LineWidth', 2);
            plt.Color = axs.ColorOrder(1,:);
            plt.HandleVisibility = 'off';
            
            plt = plot(x, 10*log10(SNR_True_Estimated), 'LineWidth', 2.5);
            plt.Color = axs.ColorOrder(2,:);
            plt = plot(xSelected, 10*log10(SNR_True_Estimated(indSelected)), 'o','MarkerSize', 12, 'LineWidth', 2);
            plt.Color = axs.ColorOrder(2,:);
            plt.HandleVisibility = 'off';
            plt = plot([0 1], 10*log10(SNR_True_Estimated(indSelected(1)))*ones(2,1), '--', 'LineWidth', 2);
            plt.Color = axs.ColorOrder(2,:);
            plt.HandleVisibility = 'off';
            grid on
            ...
            grid on
            set(gca,'FontSize',20);    
            lgd = legend({'Optimal $SNR$', '$SNR$ with uncertainty on $a(x)$'},'Interpreter', 'latex','FontSize',20);            
            lbl = xlabel('$x$','FontSize',28);
            lbl.Interpreter = 'latex';
            lbl = ylabel('$SNR$ [dB]','FontSize',28);
            lbl.Interpreter = 'latex';
...
for indPlot = 1:nbA
    ...
    plt = fill([sa2_set.^(1/2) ; flipud(sa2_set.^(1/2))], 100*[quantile(squeeze(failure_size_mont(indPlot,:,:)).', .25).' ; flipud(quantile(squeeze(failure_size_mont(indPlot,:,:)).', .75).')]/nbPts, .75*[1 1 1]); hold on
    plt.HandleVisibility = 'off';
    plt.LineStyle = 'none';
    
    plt = plot(sa2_set.^(1/2), 100*median(squeeze(failure_size_mont(indPlot,:,:)).')/nbPts); hold on
    plt.Color = axs.ColorOrder(1,:);
...
figure(5); clf
axs = axes;
plt = fill([sa2_set.^(1/2) ; flipud(sa2_set.^(1/2))], 100*[quantile(failure_size_Global.', .25).' ; flipud(quantile(failure_size_Global.', .75).')]/nbPts, .85*[1 1 1]); hold on
% plt.HandleVisibility = 'off';
plt.LineStyle = 'none';

plt = plot(sa2_set.^(1/2), 100*median(failure_size_Global.')/nbPts,'LineWidth',2.5); hold on
plt.Color = axs.ColorOrder(1,:);
...
ylabel('Failure [\%]','interpreter','latex','FontSize',28)
xlabel('$\sigma_u$ (i.e. ''Uncertainty on $a(x)$'')','interpreter','latex','FontSize',28)
