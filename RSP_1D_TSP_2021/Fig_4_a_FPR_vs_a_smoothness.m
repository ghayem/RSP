...
tic
%%
% la_set = [.01:.1:0.91,1:1:10];
nbla = 15;
la_set = logspace(-2, 1, nbla).';

ratio_rho = .1;
ind_rho = size(la_set,1);
nbMcMc = 10;
sigmaS = 1;  % power of s
...
%%
semilogx(la_set,smoothdata(smoothdata(smoothdata(failure_SNRExpectation_average))),'LineWidth',5);
hold on 
semilogx(la_set,smoothdata(smoothdata(failure_CDF_Complementary_average,1)),'LineWidth',5);
% xlim([0,100])
set(gca,'FontSize',55);
...
xlabel('$\rho_a$','interpreter','latex','FontSize',80)

set (gcf, 'Position' , [120, 150, 2000, 1000])
