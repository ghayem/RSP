...
function [a_hat_N] = Function_a_hat(params, a_K, m_K, x_K, x_N, m_N )
...
C_KK = My_2D_Cov ( rho_a, sig_a, x_K, x_K ) ;
L_C_KK = chol(C_KK + eps*eye(size(C_KK)), 'lower');


G_KK = My_2D_Cov ( rho_n, sig_n, x_K, x_K );
% L_G_KK = chol(G_KK + eps*eye(size(G_KK)), 'lower');

CG_KK = C_KK + G_KK;
L_CG_KK = chol(CG_KK + eps*eye(size(CG_KK)), 'lower');
...
C_NK = My_2D_Cov ( rho_a, sig_a, x_Ni, x_K );
...
a_hat_N(i) = m_Ni + C_NK * (  L_CG_KK.'  \   ( L_CG_KK \ ( a_K - m_K ) )  );


% C_hat_N = C_NN - C_NK * (  L_C_KK.'   \   (  L_C_KK \ C_NK' )   );
% C_hat_N = C_NN - C_NK * (  L_CG_KK.'   \   (  L_CG_KK \ C_NK' )   );
